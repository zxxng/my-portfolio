...
**1. 초기 로딩 시간 95% 단축: 서비스 마비를 해결한 Cursor 기반 페이징**

* **🔴 문제 상황:** 데이터가 5만 건 이상으로 확장되며, **10분 이상의 데이터 조회 시간**이 소요되어 서비스 사용이 불가능한 업무 마비 상태였습니다.
* **🔵 해결 과정:**
    * **Cursor 기반 페이징 기법**을 도입하여, 대규모 데이터 조회 시에도 일정한 성능을 유지하도록 구현했습니다.
    * "왜 Offset이 아닌 Cursor 방식인가?" 라는 의문에서 학습을 시작하여, 페이지가 뒤로 갈수록 성능이 저하되는 Offset 방식의 한계를 이해하고, Cursor 방식의 기술적 우수성을 체득했습니다.
    * API 요청 시 현재 페이지의 마지막 데이터 ID를 `cursor`로 전달하여, DB 스캔 범위를 최소화하는 방식으로 성능을 확보했습니다.
* **🟢 결과:** **초기 데이터 조회 시간 95% 단축:** 10분 이상 걸리던 로딩 시간을 평균 30초 이내로 개선했습니다.

**2. 사용자 경험(UX) 개선: SSE를 활용한 실시간 진행률 표시**

* **🔴 문제 상황:** 데이터 조회 시간은 단축되었으나, **저장 작업 시 여전히 긴 대기 시간**이 발생했고, 사용자는 작업이 정상 처리되는지 알 수 없었습니다.
* **🔵 해결 과정:**
    * 서버-클라이언트 단방향 데이터 스트리밍에 가장 적합한 **SSE(Server-Sent Events)**를 최종 선택했습니다. (Polling, WebSocket 비교 검토)
    * 브라우저 호환성과 인증 헤더 전송 문제를 해결하기 위해, `EventSourcePolyfill`을 조건부로 적용하여 안정성을 높였습니다.
* **🟢 결과:** 막연한 기다림을 '예측 가능한 경험'으로 전환하여, 시스템에 대한 **신뢰도와 사용성을 크게 향상**시켰습니다.

**3. UI 응답 속도 98% 개선: AG-Grid 렌더링 최적화**

* **🔴 문제 상황:** 그리드 셀의 코멘트 상태 변경 시, 단지 색상 하나를 바꾸기 위해 그리드 전체 데이터를 다시 조회하여 **30초 이상의 응답 지연**이 발생했습니다.
* **🔵 해결 과정:**
    * 전체 로직 수정의 리스크를 피하기 위해 AG-Grid 공식 문서에서 해답을 찾기로 결정, 특정 셀만 타겟하여 업데이트하는 `Cell Update API`를 발견하고 적용했습니다.
* **🟢 결과:** 30초 이상 걸리던 코멘트 관련 모든 작업의 응답 시간을 **1초 미만으로 단축**하여, 끊김 없는 사용자 경험을 제공했습니다.

**4. 클라우드 연동 매뉴얼 시스템 구축: 기획부터 UX 개선까지**

* **🔴 문제 상황:** 기존 단일 PDF 매뉴얼의 한계와, 외부 동영상 공유 시 담당자가 매번 거쳐야 하는 번거로운 DRM 결재 프로세스가 업무의 병목 지점이었습니다.
* **🔵 해결 과정:**
    * 단순 '동영상 목록' 기능을 요청한 고객사에게, **페이지 라우터 기반의 자동 매뉴얼 노출 시스템**을 역제안했습니다.
    * 보안 강화를 위해 **AWS S3 Presigned URL**을 도입하고, 편의성 극대화를 위해 **`react-player`의 PIP(Picture-in-Picture) 기능**을 추가했습니다.
* **🟢 결과:** 단순 절차 개선을 넘어, **사용 편의성, 관리 효율성, 보안까지 확보**하는 더 높은 수준의 시스템을 구축하여 고객사의 높은 만족도를 이끌어냈습니다.

**5. 아키텍처 개선: 지속 가능한 코드를 위한 고민**

* **🔴 문제 상황:** 여러 페이지의 유사 기능이 **중복 코드로 구현**되어, 작은 수정에도 모든 파일을 변경해야 하는 비효율적인 구조였습니다.
* **🔵 해결 과정:**
    * 페이지별 미묘한 차이에 대응하기 위해, 컴파운드 컴포넌트 패턴을 적용했습니다.
    * 공통 로직은 부모 컴포넌트가, 각기 다른 UI는 자식 컴포넌트가 담당하게 하여, **유연하고 확장 가능한 구조**를 완성했습니다.
* **🟢 결과:** 로직 수정 시 단 한 곳만 수정하면 일괄 적용되도록 개선하여, **유지보수성을 향상**하였습니다.
